...
; (function (window) {
   ...
   * @param putPolicy 上传策略
   */
  var genUpToken = (accessKey, secretKey, putPolicy) => {
    //Step1 确定上传策略，也就是putPolicy的值
    //Step2 将上传策略序列化为JSON格式
    var putPpolicyStr = JSON.stringify(putPolicy);
    //Step3 对字符串化的安全策略进行URL安全的base64编码
    var encoded = base64encode(utf16to8(putPpolicyStr));
    //Step4 用secretKey对encoded进行HMAC-SHA1加密，然后再做URL安全的base64编码
    var hash = CryptoJS.HmacSHA1(encoded, secretKey);
    var encodedSigned = hash.toString(CryptoJS.enc.Base64); 
    //Step5 将accessKey,encodedSigned,encoded用":"(冒号)连接起来，得到Uptoken
